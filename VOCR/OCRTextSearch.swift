...
class OCRTextSearch {
    ...
    func search(query: String = "", fromBeginning: Bool = false, backward: Bool = false) {
        ...
        var startIndex =
            (Navigation.l >= 0 && Navigation.l < Navigation.displayResults.count) ? Navigation.l : 0
        var startWordIndex =
            (startIndex >= 0 && startIndex < Navigation.displayResults.count && Navigation.w >= 0
                && Navigation.w < Navigation.displayResults[startIndex].count) ? Navigation.w : 0
        ...
        if fromBeginning {
            ...
            if backward {
                ...
                startWordIndex += 1
                if startIndex < Navigation.displayResults.count
                    && startWordIndex >= Navigation.displayResults[startIndex].count
                {
                    startIndex += 1
                    startWordIndex = 0
                }
        ...
        if backward {
            ...
            while lineIndex >= 0 {
                ...
                for wordIndex in stride(
                    from: lineIndex == startIndex ? startWordIndex : wordCount - 1, through: 0,
                    by: -1)
                {
                    if isMatch(line: line, wordIndex: wordIndex) {
                        ...
                        Accessibility.speak(
                            "\(Navigation.displayResults[lineIndex][wordIndex].value)")
                        return
                    }
                }
                lineIndex -= 1
        ...
        Accessibility.speak(
            NSLocalizedString(
                "search.not_found", value: "Not found.",
                comment: "Message when search text is not found"))
    }
    ...
    func showSearchDialog() {
        ...
        alert.messageText = NSLocalizedString(
            "search.dialog.title", value: "Search OCR Text", comment: "Title for the search dialog")
        ...
        alert.addButton(
            withTitle: NSLocalizedString(
                "button.from_beginning", value: "From Beginning",
                comment: "Button to search from the beginning"))
        alert.addButton(
            withTitle: NSLocalizedString(
                "button.from_current", value: "From Current",
                comment: "Button to search from current position"))
        alert.addButton(
            withTitle: NSLocalizedString(
                "button.cancel", value: "Cancel", comment: "Button to cancel the search"))
        ...
        if response == .alertFirstButtonReturn {  // From Beginning
            ...
            Accessibility.speak(
                NSLocalizedString(
                    "search.cancelled", value: "Cancelled.",
                    comment: "Message when search is cancelled"))
            return
        }
        sleep(1)
    }
    ...
    private func setMouseCoordinates(x: Int, y: Int) {
        if Settings.moveMouse {
            CGWarpMouseCursorPosition(
                Navigation.convert2coordinates(Navigation.displayResults[x][y].boundingBox))
        }
