...
enum Settings {
    ...
    static var allSettings: [(title: String, action: Selector, value: Bool)] {
        return [
            (NSLocalizedString("menu.settings.targetWindow", value: "Target Window", comment: "Menu item for selecting target window"), #selector(MenuHandler.toggleSetting(_:)), targetWindow),
            (NSLocalizedString("menu.settings.autoScan", value: "Auto Scan", comment: "Menu item for enabling auto scan on mouse click"), #selector(MenuHandler.toggleAutoScan(_:)), autoScan),
            (NSLocalizedString("menu.settings.detectObjects", value: "Detect Objects", comment: "Menu item for enabling object detection"), #selector(MenuHandler.toggleSetting(_:)), detectObject),
            (NSLocalizedString("menu.settings.usePresetPrompt", value: "Use Preset Prompt", comment: "Menu item for using preset prompt with AI"), #selector(MenuHandler.toggleSetting(_:)), usePresetPrompt),
            (NSLocalizedString("menu.settings.resetPosition", value: "Reset Position on Scan", comment: "Menu item for resetting cursor position after each scan"), #selector(MenuHandler.toggleSetting(_:)), positionReset),
            (NSLocalizedString("menu.settings.positionalAudio", value: "Positional Audio", comment: "Menu item for enabling spatial audio feedback"), #selector(MenuHandler.toggleSetting(_:)), positionalAudio),
            (NSLocalizedString("menu.settings.moveMouse", value: "Move Mouse", comment: "Menu item for moving mouse cursor during navigation"), #selector(MenuHandler.toggleSetting(_:)), moveMouse),
            (NSLocalizedString("menu.settings.launchOnLogin", value: "Launch on Login", comment: "Menu item for launching VOCR on system startup"), #selector(MenuHandler.toggleLaunch(_:)), launchOnBoot),
            (NSLocalizedString("menu.settings.log", value: "Log", comment: "Menu item for enabling debug logging"), #selector(MenuHandler.toggleLaunch(_:)), writeLog),
        ]
    }
    ...
    static func setupMenu() -> NSMenu {
        ...
        let presetsMenuItem = NSMenuItem(title: NSLocalizedString("menu.presets", value: "Presets", comment: "Menu item for preset management"), action: nil, keyEquivalent: "")
        ...
        let soundOutputMenuItem = NSMenuItem(
            title: NSLocalizedString("menu.soundOutput", value: "Sound Output...", comment: "Menu item for choosing audio output device"), action: #selector(target.chooseOutput(_:)), keyEquivalent: "")
        soundOutputMenuItem.target = target
        settingsMenu.addItem(soundOutputMenuItem)

        let cameraMenuItem = NSMenuItem(
            title: NSLocalizedString("menu.chooseCamera", value: "Choose Camera...", comment: "Menu item for selecting camera device"), action: #selector(target.chooseCamera(_:)), keyEquivalent: ""
        )
        cameraMenuItem.target = target
        settingsMenu.addItem(cameraMenuItem)

        let shortcutsMenuItem = NSMenuItem(
            title: NSLocalizedString("menu.shortcuts", value: "Shortcuts...", comment: "Menu item for managing keyboard shortcuts"), action: #selector(target.openShortcutsWindow(_:)),
            keyEquivalent: "")
        shortcutsMenuItem.target = target
        settingsMenu.addItem(shortcutsMenuItem)

        let newShortcutMenuItem = NSMenuItem(
            title: NSLocalizedString("menu.newShortcuts", value: "New Shortcuts", comment: "Menu item for creating a new keyboard shortcut"), action: #selector(target.addShortcut(_:)), keyEquivalent: "")
        newShortcutMenuItem.target = target
        //		settingsMenu.addItem(newShortcutMenuItem)

        let resetMenuItem = NSMenuItem(
            title: NSLocalizedString("menu.reset", value: "Reset...", comment: "Menu item for resetting all settings and presets"), action: #selector(target.reset(_:)), keyEquivalent: ""
        )
        resetMenuItem.target = target
        settingsMenu.addItem(resetMenuItem)

        let settingsMenuItem = NSMenuItem(title: NSLocalizedString("menu.settings", value: "Settings", comment: "Main settings menu item"), action: nil, keyEquivalent: "")
        settingsMenuItem.submenu = settingsMenu
        menu.addItem(settingsMenuItem)

        if Navigation.cgImage != nil {
            let saveScreenshotMenuItem = NSMenuItem(
                title: NSLocalizedString("menu.saveLatestImage", value: "Save Latest Image", comment: "Menu item for saving the most recent captured image"), action: #selector(target.saveLastImage(_:)),
                keyEquivalent: "s")
        ...
        if Navigation.displayResults.count > 1 {
            let saveMenuItem = NSMenuItem(
                title: NSLocalizedString("menu.saveOCRResult", value: "Save OCR Result...", comment: "Menu item for saving OCR text results to file"), action: #selector(target.saveResult(_:)),
                keyEquivalent: "")
        ...
        let aboutMenuItem = NSMenuItem(
            title: NSLocalizedString("menu.about", value: "About...", comment: "Menu item for displaying About window"), action: #selector(target.displayAboutWindow(_:)), keyEquivalent: "")
        aboutMenuItem.target = target
        updateMenu.addItem(aboutMenuItem)

        let checkForUpdatesItem = NSMenuItem(
            title: NSLocalizedString("menu.checkForUpdates", value: "Check for Updates", comment: "Menu item for manually checking for software updates"), action: #selector(target.checkForUpdates), keyEquivalent: ""
        )
        checkForUpdatesItem.target = target
        updateMenu.addItem(checkForUpdatesItem)

        let autoCheckItem = NSMenuItem(
            title: NSLocalizedString("menu.autoCheckForUpdates", value: "Automatically Check for Updates", comment: "Menu item for enabling automatic update checks"), action: #selector(target.toggleSetting(_:)),
            keyEquivalent: "")
        autoCheckItem.target = target
        updateMenu.addItem(autoCheckItem)

        let autoUpdateItem = NSMenuItem(
            title: NSLocalizedString("menu.autoInstallUpdates", value: "Automatically Install Updates", comment: "Menu item for enabling automatic update installation"), action: #selector(target.toggleSetting(_:)),
            keyEquivalent: "")
        ...
        let preReleaseItem = NSMenuItem(
            title: NSLocalizedString("menu.downloadPreRelease", value: "Download  Pre-release", comment: "Menu item for enabling pre-release version downloads"), action: #selector(target.toggleSetting(_:)),
            keyEquivalent: ""
        )
        ...
        let updateMenuItem = NSMenuItem(title: NSLocalizedString("menu.updates", value: "Updates", comment: "Updates menu section"), action: nil, keyEquivalent: "")
        updateMenuItem.submenu = updateMenu
        menu.addItem(updateMenuItem)

        if Shortcuts.navigationActive {
            let dismissMenuItem = NSMenuItem(
                title: NSLocalizedString("menu.dismiss", value: "Dismiss Menu", comment: "Menu item for dismissing the menu"), action: #selector(target.dismiss(_:)), keyEquivalent: "z")
        ...
        menu.addItem(
            NSMenuItem(
                title: NSLocalizedString("menu.quit", value: "Quit", comment: "Menu item for quitting the application"), action: #selector(NSApplication.terminate(_:)), keyEquivalent: "q"))
        return menu
    }
    ...
    private static func buildPresetsSubmenu(into submenu: NSMenu) {
        ...
        let presetManagerItem = NSMenuItem(
            title: NSLocalizedString("menu.presetManager", value: "Preset Manager…", comment: "Menu item for opening preset manager window"),
        ...
        let editExplorePromptsItem = NSMenuItem(
            title: NSLocalizedString("menu.editExplorePrompts", value: "Edit Explore Prompts…", comment: "Menu item for editing explore mode prompts"),
...
class MenuHandler: NSObject {
    @objc func toggleSetting(_ sender: NSMenuItem) {
        ...
        switch sender.title {
        case NSLocalizedString("menu.settings.targetWindow", value: "Target Window", comment: "Menu item for selecting target window"):
            Settings.targetWindow = sender.state == .on
        case NSLocalizedString("menu.settings.detectObjects", value: "Detect Objects", comment: "Menu item for enabling object detection"):
            Settings.detectObject = sender.state == .on
        case NSLocalizedString("menu.settings.autoScan", value: "Auto Scan", comment: "Menu item for enabling auto scan on mouse click"):
            Settings.autoScan = sender.state == .on
        case NSLocalizedString("menu.settings.resetPosition", value: "Reset Position on Scan", comment: "Menu item for resetting cursor position after each scan"):
            Settings.positionReset = sender.state == .on
        case NSLocalizedString("menu.settings.positionalAudio", value: "Positional Audio", comment: "Menu item for enabling spatial audio feedback"):
            Settings.positionalAudio = sender.state == .on
        case NSLocalizedString("menu.settings.usePresetPrompt", value: "Use Preset Prompt", comment: "Menu item for using preset prompt with AI"):
            Settings.usePresetPrompt = sender.state == .on
        case NSLocalizedString("menu.settings.moveMouse", value: "Move Mouse", comment: "Menu item for moving mouse cursor during navigation"):
            Settings.moveMouse = sender.state == .on
        case NSLocalizedString("menu.settings.launchOnLogin", value: "Launch on Login", comment: "Menu item for launching VOCR on system startup"):
            Settings.launchOnBoot = sender.state == .on
        case NSLocalizedString("menu.settings.log", value: "Log", comment: "Menu item for enabling debug logging"):
            Settings.writeLog = sender.state == .on
        case NSLocalizedString("menu.downloadPreRelease", value: "Download  Pre-release", comment: "Menu item for enabling pre-release version downloads"):
            Settings.preRelease = sender.state == .on
        case NSLocalizedString("menu.autoCheckForUpdates", value: "Automatically Check for Updates", comment: "Menu item for enabling automatic update checks"):
            if let updater = AutoUpdateManager.shared.updaterController?.updater {
                updater.automaticallyChecksForUpdates = sender.state == .on
            }
        case NSLocalizedString("menu.autoInstallUpdates", value: "Automatically Install Updates", comment: "Menu item for enabling automatic update installation"):
            if let updater = AutoUpdateManager.shared.updaterController?.updater {
    ...
    @objc func chooseOutput(_ sender: Any?) {
        ...
        alert.messageText = NSLocalizedString("dialog.soundOutput.title", value: "Sound Output", comment: "Title for sound output selection dialog")
        alert.informativeText = NSLocalizedString("dialog.soundOutput.message", value: "Choose an Output for positional audio feedback.", comment: "Message prompting user to select audio output device")
    ...
    @objc func chooseCamera(_ sender: Any?) {
        ...
        if devices.count > 1 {
            let alert = NSAlert()
            alert.alertStyle = .informational
            alert.messageText = NSLocalizedString("dialog.camera.title", value: "Camera", comment: "Title for camera selection dialog")
            alert.informativeText = NSLocalizedString("dialog.camera.message", value: "Choose a camera for VOCR to use.", comment: "Message prompting user to select camera device")
            for device in devices {
    ...
    @objc func addShortcut(_ sender: NSMenuItem) {
        let alert = NSAlert()
        alert.messageText = NSLocalizedString("dialog.newShortcut.title", value: "New Shortcut", comment: "Title for new shortcut creation dialog")
        alert.addButton(withTitle: NSLocalizedString("button.create", value: "Create", comment: "Button to create a new item"))
        alert.addButton(withTitle: NSLocalizedString("button.cancel", value: "Cancel", comment: "Button to cancel an action"))
        let inputTextField = NSTextField(frame: NSRect(x: 0, y: 0, width: 200, height: 24))
        inputTextField.placeholderString = NSLocalizedString("dialog.newShortcut.placeholder", value: "Shortcut Name", comment: "Placeholder text for shortcut name input field")
        ...
        if response == .alertFirstButtonReturn {  // OK button
            Shortcuts.shortcuts.append(
                Shortcut(
                    ...
                    keyName: NSLocalizedString("dialog.newShortcut.unassigned", value: "Unassigned", comment: "Status text for unassigned keyboard shortcut")))
    ...
    @objc func openEditExplorePrompts(_ sender: NSMenuItem) {
        let alert = NSAlert()
        alert.alertStyle = .informational
        alert.messageText = NSLocalizedString("dialog.editExplorePrompts.title", value: "Edit Explore Prompts", comment: "Title for explore prompts editing dialog")
        alert.informativeText = NSLocalizedString("dialog.editExplorePrompts.message", value: "Update the system and user prompts used for the Explore Mode.", comment: "Description of explore prompts editing functionality")
        alert.addButton(withTitle: NSLocalizedString("button.save", value: "Save", comment: "Button to save changes"))
        alert.addButton(withTitle: NSLocalizedString("button.cancel", value: "Cancel", comment: "Button to cancel an action"))
        // Avoid Return triggering the default button; keep Return for newline in text views.
        alert.buttons.first?.keyEquivalent = ""
        alert.buttons.last?.keyEquivalent = "\u{1b}"

        let systemLabel = NSTextField(labelWithString: NSLocalizedString("dialog.editExplorePrompts.systemPromptLabel", value: "System Prompt", comment: "Label for system prompt text field"))
        ...
        let userLabel = NSTextField(labelWithString: NSLocalizedString("dialog.editExplorePrompts.userPromptLabel", value: "User Prompt", comment: "Label for user prompt text field"))
    ...
    @objc func reset(_ sender: NSMenuItem) {
        let alert = NSAlert()
        alert.messageText = NSLocalizedString("dialog.reset.title", value: "Reset?", comment: "Title for reset confirmation dialog")
        alert.informativeText = NSLocalizedString("dialog.reset.message", value: "This will erase all the settings and presets. This cannot be undone.", comment: "Warning message about irreversible reset action")
        alert.alertStyle = .warning

        alert.addButton(withTitle: NSLocalizedString("button.resetAndRelaunch", value: "Reset and Relaunch", comment: "Button to reset settings and restart application"))  // first button is return value .alertFirstButtonReturn
        alert.addButton(withTitle: NSLocalizedString("button.cancel", value: "Cancel", comment: "Button to cancel an action"))
